...
const addProduct = async (req, res) => {
  try {
    ...
    const q = `
      ...
      RETURNING *;
    `;
    const result = await pool.query(q, [name, description, price, stock, vendorId]);
...
const getVendorProducts = async (req, res) => {
  try {
    const vendorId = req.vendorId;
    if (!vendorId) return res.status(401).json({ error: 'Vendor authentication failed' });
...
const updateProduct = async (req, res) => {
  try {
    ...
    const q = `
      ...
      RETURNING *;
    `;
    const result = await pool.query(q, [name, description, price, stock, id, vendorId]);
    if (!result.rows[0]) return res.status(404).json({ error: 'Product not found or not yours' });
